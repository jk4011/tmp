...
class Run(object):
    ...
    def init_network(self):
        ...
        if (args.RAdam):
            ...
            self.d_optimizer = optim2.RAdam(self.D.parameters(), lr=1e-4, betas=(0.9, 0.999), eps=1e-8, weight_decay=0)
        else:
    ...
    def train(self):
        ...
        for epoch in range(start,self.args.num_epochs):
            ...
            for i, real_images in enumerate(tqdm(self.data_loader)): #real_image: B x 3 x H x W
                # real_images = torch.tensor(real_images)
                edge = torch.zeros_like(torch.tensor(real_images))[:,0:1,:,:].type(torch.float32)
